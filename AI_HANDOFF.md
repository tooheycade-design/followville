...
## Current town (Day 22, 2026-07-23)

Population is 352 with 406 buildings. Day 22 added 10 ordinary claimable
Meadow Run homes (plan IDs 199-208, seeds 397-406) plus Fire Station 1
(seed 396, non-claimable). The full-block station is centered at Blender
`(64.5,-70.5)` / website `(64.5,70.5)` and replaces only the procedural tree
in that empty downtown block; no prior building record was removed. All 395
pre-Day-22 records are byte-identical and address 209 is next. Supabase added
the 11 new rows once and final replay syncs inserted zero. Full/fallback plus
eight streamed chunks pass exact state, hash, 406-building coverage, and
walk-surface validation. The authoritative and mirror Blends are saved at
finished frame 420. The reviewed Day 22 delivery is one 14.000-second daytime
portrait MP4 generated by `--cam day22reveal`: skyline, flight to the ten-home
rise, then flight to the empty civic block before the station rises.
