-- Recover a content task that failed before the trusted content runtime existed.
-- The service-only guard accepts only a failed private-preview task whose last
-- worker attempt changed no files. It cannot reopen arbitrary failed work.
...
begin
  ...
  if not found
     ...
     or cardinality(v_task.allowed_capabilities) <> 5
     or not (
     ...
     or not exists (
       select 1
        ...
        where packet.production_task_id = v_task.id
          and packet.status = 'production_in_progress'
     ) then
    return false;
  end if;
