...
2026-07-17 - Cade (via Windows Codex) - [WEB] Published an Avatar System v1 correction that keeps only the single modular animated character family by removing the separate tall complete-character catalog/assets and makes the production streamed-town camera initialize correctly and rebuild from the player's live position every gameplay frame; old saved look IDs fall back safely, a long-run camera-lock regression, the focused avatar flows, and full-town fallback passed, while Day 16 state, population, buildings, claims, ownership, town assets, and Blender were untouched.

2026-07-17 - Cade (via Windows Codex) - [WEB] Released third-person Avatar System v1: a reference-matched Neighborhood Tailor, modular body/face/hair/outfit/hat choices, 37 additional complete looks with real preview images, lazy 3D loading, guest persistence, and verified owner-only profile persistence; desktop/mobile visual QA, 50 asset hashes, all 10 browser flows, authenticated/unauthorized database tests, and production checks passed without changing Day 16 town state, population, geometry, any of the 30 claims, or ownership.
