...
2026-07-17 - Cade (via Windows Codex) - [WEB] Corrected Avatar System v1 around the approved compact animated characters: the public Tailor now offers only the 37 complete characters plus body color/height, legacy tall modular selections normalize safely and no modular GLB loads, the follow rig initializes in streamed/full paths, desktop uses cursor-locked right-drag orbit plus wheel/trackpad zoom into first person, mobile supports camera drag while moving plus pinch zoom, and camera-relative A/D are corrected. Bare legacy `town.html` entry now returns to the current map-preview/Today homepage. Focused desktop/mobile/fallback browser flows passed without changing Day 16 state, population, geometry, claims, ownership, Supabase data, or Blender assets.
