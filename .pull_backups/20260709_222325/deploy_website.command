...
cd "$REPO"
# 2026-07-10: explicit checkout before pulling -- this clone is now shared
# with pull_latest.command/share_progress.command, which switch it to other
# branches (main, wip). Without this, "git pull origin main" would pull INTO
# whatever branch the clone happened to be left on by the last script run.
git checkout main
